...
public class DetailPostPageRepo implements IDetailPostPageRepo {
    ...
    private com.hm.socialmedia.tables.Reply reply = com.hm.socialmedia.tables.Reply.REPLY;
    ...
    public boolean updatePostCommentCount(Integer id) {
        ...
        return dslContext.update(posts)
                .set(posts.COMMENT_COUNT, posts1.getCommentCount() + 1)
                .where(posts.ID.eq(id))
                .execute() == 1;
    ...
    public Reply insertReply(Reply reply1) {
        ...
        ReplyRecord replyRecord = dslContext.insertInto(reply, reply.CONTENT,reply.COMMENT_ID,
                        reply.POST_ID,reply.CREATED_AT,reply.COMMENT_USER_ID, reply.USER_OWNER_ID
                        ,reply.VOTE_COUNT,reply.POST_USER_ID)
                .values(reply1.getContent(), reply1.getCommentId()
                        ,reply1.getPostId(), LocalDateTime.now(), reply1.getCommentUserId()
                        ,reply1.getUserOwnerId(),0,reply1.getPostUserId())
                .returning(reply.ID)
                .fetchOne();
